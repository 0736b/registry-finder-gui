...
func main() {
	...
	rr := repositories.NewRegistryRepository()

	streamReg := rr.StreamRegistry()

	for reg := range streamReg {
		log.Println(reg)
	}

	// hkey, _ := registry.OpenKey(registry.LOCAL_MACHINE, "HARDWARE", registry.READ)
	// defer hkey.Close()

	// QueryEnumKeys(hkey, "HKEY_LOCAL_MACHINE\\HARDWARE")
	// log.Println(results)
